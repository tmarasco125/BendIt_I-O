...
class BenditHub {
    constructor() {
        ...
        this.collectAssignedDeviceData = this.collectAssignedDeviceData.bind(this);
        this.removeDeviceOnDisconnect = this.removeDeviceOnDisconnect.bind(this);
        this.version = 1.0;
    ...
    postStats(type) {
        ...
        switch (inquiry) {
            ...
            case "devices":
                data = {
                    "total_devices": this.connectedBenditBoards.length,
                };
                console.log(JSON.stringify(data));
        }
    ...
    collectAssignedDeviceData(deviceData) {
        ...
        this.connectedBenditBoards.push(deviceData);
    ...
    initServer() {
        ...
        io.on('connection', (socket) => {
            ...
            socket.on('potEvent', (data)=>{
                console.log(`Device ${data.device} was told to set pot ${data.pot_number} to position ${data.position} from ${socket.id}`);
            ...
            socket.on('handshake', (data) => {
                ...
                let deviceAssignedColor = this.deviceColors[Math.floor(Math.random() * this.deviceColors.length)];


                setTimeout(function () {
                    //do these two need to be io instead of socket?
                    io.to(`${socket.id}`).emit("setDeviceColor", deviceAssignedColor);
                    console.log(`here is the color message sent to the Bendit board: ${deviceAssignedColor}`);
                    io.to(`${socket.id}`).emit("setDeviceNumber", socket.deviceNumber);
                ...
                this.connectedBenditBoards.push(socket.deviceAssignedData);
